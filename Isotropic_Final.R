...
RD_eq = 0.08*D_num_links

RA_eq = 0.08*A_num_links
...
for(j in 1:num_paths){
  ...
  Ds1[1] = Dx_coord[1]/D_norm[1]
  Ds2[1] = Dy_coord[1]/D_norm[1]
  ...
  As1[1] = Ax_coord[1]/A_norm[1]
  As2[1] = Ay_coord[1]/A_norm[1]
  As3[1] = Az_coord[1]/A_norm[1]
  ...
  Ao2[1] = rnorm(1,0,1)
  Ao3[1] = rnorm(1,0,1)
  Ao_N[1] = (Ao1[1]^2 + Ao2[1]^2 + Ao3[1]^2)
  
  Ao1_N[1] = Ao1[1]/Ao_N[1]
  Ao2_N[1] = Ao2[1]/Ao_N[1]
  Ao3_N[1] = Ao3[1]/Ao_N[1]
  
  
  Do1[1] = rnorm(1,0,1)
  Do2[1] = rnorm(1,0,1)
  Do3[1] = rnorm(1,0,1)
  Do_N[1] = (Do1[1]^2 + Do2[1]^2 + Do3[1]^2)
  
  Do1_N[1] = Do1[1]/Do_N[1]
  Do2_N[1] = Do2[1]/Do_N[1]
  Do3_N[1] = Do3[1]/Do_N[1]
  ...
  kappa[1] = ((Ao1_N[1]*Do1_N[1] + Ao2_N[1]*Do2_N[1] + Ao3_N[1]*Do3_N[1]) - 3*(R_hat1[1]*Do1_N[1] + R_hat2[1]*Do2_N[1] + R_hat3[1]*Do3_N[1])*(R_hat1[1]*Ao1_N[1] + R_hat2[1]*Ao2_N[1] + R_hat3[1]*Ao3_N[1]))^2
  
  DA[1] = df*(3/2)*kappa[1]*(R_0/Sp_Dist[1])^6
  ...
  for (i in 2:n) {
    ...
    R_D[i] = R_D[i-1] + Dk_hat*(RD_eq - R_D[i-1])*Delta + sqrt(2*D_Diff)*rnorm(1,0,sd=sqrt(Delta))
    
    R_A[i] = R_A[i-1] + Ak_hat*(RA_eq - R_A[i-1])*Delta + sqrt(2*A_Diff)*rnorm(1,0,sd=sqrt(Delta))
    
    Dx_coord[i] = Dx_coord[i-1] + Dsk*(D1 - Dx_coord[i-1])*Delta + sqrt(2*D_Diff)*rnorm(1,0,sd=sqrt(Delta))
    Dy_coord[i] = Dy_coord[i-1] + Dsk*(D2 - Dy_coord[i-1])*Delta + sqrt(2*D_Diff)*rnorm(1,0,sd=sqrt(Delta))
    Dz_coord[i] = Dz_coord[i-1] + Dsk*(D3 - Dz_coord[i-1])*Delta + sqrt(2*D_Diff)*rnorm(1,0,sd=sqrt(Delta))
    
    D_norm[i] = sqrt(Dx_coord[i]^2 + Dy_coord[i]^2 + Dz_coord[i]^2)
    
    Ds1[i] = Dx_coord[i]/D_norm[i]
    Ds2[i] = Dy_coord[i]/D_norm[i]
    Ds3[i] = Dz_coord[i]/D_norm[i]
    
    Ax_coord[i] = Ax_coord[i-1] + Ask*(A1 - Ax_coord[i-1])*Delta + sqrt(2*A_Diff)*rnorm(1,0,sd=sqrt(Delta))
    Ay_coord[i] = Ay_coord[i-1] + Ask*(A2 - Ay_coord[i-1])*Delta + sqrt(2*A_Diff)*rnorm(1,0,sd=sqrt(Delta))
    Az_coord[i] = Az_coord[i-1] + Ask*(A3 - Az_coord[i-1])*Delta + sqrt(2*A_Diff)*rnorm(1,0,sd=sqrt(Delta))
    
    A_norm[i] = sqrt(Ax_coord[i]^2 + Ay_coord[i]^2 + Az_coord[i]^2)
    
    As1[i] = Ax_coord[i]/A_norm[i]
    As2[i] = Ay_coord[i]/A_norm[i]
    As3[i] = Az_coord[i]/A_norm[i]
    ...
    Ao2[i] = rnorm(1,0,1)
    Ao3[i] = rnorm(1,0,1)
    Ao_N[i] = sqrt((Ao1[i]^2 + Ao2[i]^2 + Ao3[i]^2))
    
    Ao1_N[i] = Ao1[i]/Ao_N[i]
    Ao2_N[i] = Ao2[i]/Ao_N[i]
    Ao3_N[i] = Ao3[i]/Ao_N[i]
    
    
    Do1[i] = rnorm(1,0,1)
    Do2[i] = rnorm(1,0,1)
    Do3[i] = rnorm(1,0,1)
    Do_N[i] = sqrt((Do1[i]^2 + Do2[i]^2 + Do3[i]^2))
    
    Do1_N[i] = Do1[i]/Do_N[i]
    Do2_N[i] = Do2[i]/Do_N[i]
    Do3_N[i] = Do3[i]/Do_N[i]
    
    T_FRET[i] = 1/(1 + (Sp_Dist[i]/R_0)^6) #Theoretical FRET
    
    #Define Kappa for this photon
    kappa[i] = ((Ao1_N[i]*Do1_N[i] + Ao2_N[i]*Do2_N[i] + Ao3_N[i]*Do3_N[i]) - 3*(R_hat1[i]*Do1_N[i] + R_hat2[i]*Do2_N[i] + R_hat3[i]*Do3_N[i])*(R_hat1[i]*Ao1_N[i] + R_hat2[i]*Ao2_N[i] + R_hat3[i]*Ao3_N[i]))^2
    
    DA[i] = df*(3/2)*kappa[i]*(R_0/Sp_Dist[i])^6 #Energy transfer rate with Kappa - Shift Observed
...
outfile = paste("Isotropic.rds",sep = "")
