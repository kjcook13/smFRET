...
kT = 4.1*10^(-12) #0.0041  #pN microns (room temp) (nm) (10^-12 added to convert to N*nm)
...
Ak_Diff = kT/(8*pi*nu*A_Radius^3)
Dk_Diff = kT/(8*pi*nu*D_Radius^3)

#Spring constant on each dye N*nm
D_num_links = 3
A_num_links = 3

Dk_spring = (1010/D_num_links)*10^(-9) #Donor spring  1010
Ak_spring = (1010/A_num_links)*10^(-9) #Acceptor 1010
...
for(j in 1:num_paths){
  ...
  for (i in 2:n) {
    ...
    ### CTMC Photon Radiation or FRET Process 
    ##### CTMC Stuff #####
    Rad_time = 10^-6  #Time step for integral
    Rad_step = 10000
    ...
    A_theta[1] = runif(1,-pi/2,pi/2)
    D_theta[1] = runif(1,-pi/2,pi/2)
    
    A_phi[1] = runif(1,0,2*pi)
    D_phi[1] = runif(1,0,2*pi)
    
    Ao1_N[1] = sin(A_theta[1])*cos(A_phi[1])
    Ao2_N[1] = sin(A_theta[1])*sin(A_phi[1])
    Ao3_N[1] = cos(A_theta[1])
    
    Do1_N[1] = sin(D_theta[1])*cos(D_phi[1])
    Do2_N[1] = sin(D_theta[1])*sin(D_phi[1])
    Do3_N[1] = cos(D_theta[1])
    
    kappa[1] = ((Ao1_N[1]*Do1_N[1] + Ao2_N[1]*Do2_N[1] + Ao3_N[1]*Do3_N[1]) - 3*(R_hat1[i]*Do1_N[1] + R_hat2[i]*Do2_N[1] + R_hat3[i]*Do3_N[1])*(R_hat1[i]*Ao1_N[1] + R_hat2[i]*Ao2_N[1] + R_hat3[i]*Ao3_N[1]))^2
    
    
    for (k in 2:Rad_step) {
      
      #Orientation of Dipole Moments on Radiation Timescale
      A_theta[k] = A_theta[k-1] + ((Ak_Diff^2)/(2*tan(A_theta[k-1])))*Rad_time +sqrt(2*Ak_Diff)*rnorm(1,0,sd=sqrt(Rad_time))
      D_theta[k] = D_theta[k-1] + ((Dk_Diff^2)/(2*tan(D_theta[k-1])))*Rad_time +sqrt(2*Dk_Diff)*rnorm(1,0,sd=sqrt(Rad_time))
      
      A_phi[k] = (A_phi[k-1] + (Ak_Diff/sin(A_theta[k-1]))*rnorm(1,0,sd=sqrt(Rad_time)))%%(2*pi)
      D_phi[k] = (D_phi[k-1] + (Dk_Diff/sin(D_theta[k-1]))*rnorm(1,0,sd=sqrt(Rad_time)))%%(2*pi)
      
      Ao1_N[k] = sin(A_theta[k])*cos(A_phi[k])
      Ao2_N[k] = sin(A_theta[k])*sin(A_phi[k])
      Ao3_N[k] = cos(A_theta[k])
      
      Do1_N[k] = sin(D_theta[k])*cos(D_phi[k])
      Do2_N[k] = sin(D_theta[k])*sin(D_phi[k])
      Do3_N[k] = cos(D_theta[k])
      
      
      #Energy Transfer and Kappa Dynamics
      kappa[k] = ((Ao1_N[k]*Do1_N[k] + Ao2_N[k]*Do2_N[k] + Ao3_N[k]*Do3_N[k]) - 3*(R_hat1[i]*Do1_N[k] + R_hat2[i]*Do2_N[k] + R_hat3[i]*Do3_N[k])*(R_hat1[i]*Ao1_N[k] + R_hat2[i]*Ao2_N[k] + R_hat3[i]*Ao3_N[k]))^2
      
    }
    
    X = rexp(1,1) #Exponential for donor fluorescence 
    Y = rexp(1,1) #Exponential for energy transfer
    ...
    J = seq(0,Rad_step-1,1)
    DF_J = df*Rad_time*J
    K_J = df*(3/2)*(kappa[(J+1)]*(R_0/Sp_Dist[i])^6)*Rad_time
    DA_J = cumsum(K_J)
    DF_J_time = min(which((Y - DF_J) <0))
    DA_J_time = min(which((X - DA_J) <0))
    
    KDA = DA_J_time*Rad_time
    KD = DF_J_time*Rad_time
...
outfile = paste("ElasticPendulum_WithKappa.rds",sep = "")
